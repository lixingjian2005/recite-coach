...
| 部分 | 作用 |
|------|------|
| Skill workflow | 指导 Codex/Claude 等 agent 把 PDF、讲义、笔记压缩成踩分点背诵清单，再生成 `cards.json` |
...
安装 Skill 后，agent 会把播放器启动包复制到你的输出目录。你不需要把 `cards.json` 放回 Skill 安装目录。
...
The Skill side turns PDFs, lecture notes, textbooks, tables, or pasted text into a score-point memorization list, then into `cards.json`. The local player reads that JSON file and runs a simple Leitner-style review session.
...
4. The agent should write `cards.json` to your current project root or a directory you name.
5. The agent should copy the local player kit into the same directory.
...
`cards.json` uses a small schema:
...
- `cards.json` may contain private notes. Check it before publishing or sharing.
